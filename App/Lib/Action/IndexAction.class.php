...
class IndexAction extends Action {
	...
	public function checkLogin(){
		...
		//var_dump($model -> where(array('username'=>'zhuyunfei6')) -> select());exit;
		$map['username'] = $_POST['username'];
	...
	public function getSetResult2(){
		...
		if($res){
			$this -> success('修改成功',U('Index/select'));
		}else{
			$this -> error('修改失败');
		}
	...
	public function getSetResult(){
		...
		$res = $model -> add($data);
		if($res){
			$this -> success('设置成功',U('Index/select'));
		}else{
			$this -> error('设置失败');
		}
	...
	public function getNewCode(){
		//http://localhost/feifei/index.php/Index/getNewCode/wzname/账号/wzpwe/设备/pass/密码/game/  WARZ/COK
		
		if($_GET['game'] == 'WARZ'){
			$model = M('2note');
		}elseif($_GET['game'] == 'COK'){
			$model = M('note');
		}elseif($_GET['game'] == 'WAO'){
			$model = M('3note');
		}
	...
	public function getCode(){
		//http://localhost/feifei/index.php/Index/getCode/wzname/123/wzpwe/1/pass/
		$model = M('note');
		$model_user = M('userinfo');
		$map['username'] = $_GET['wzname'];
	...
	function getChangePassResult(){
		$model = M('userinfo');
		if($_POST['username'] == '' || $_POST['password'] == '' || $_POST['repassword'] == '' || $_POST['mail'] == ''){
			//$this -> assign('jumpUrl','reg');
			$this -> assign('waitSecond','2');
			$this -> error('请输入必填项');
		}
