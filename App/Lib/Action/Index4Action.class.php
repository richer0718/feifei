<?php
// 本类由系统自动生成，仅供测试用途
class Index4Action extends Action {
	...
	public function getSetResult2(){
		...
		if($res){
			$this -> success('修改成功',U('Index4/select'));
		}else{
			$this -> error('修改失败');
		}
	...
	public function getSetResult(){
		...
		$res = $model -> add($data);
		if($res){
			$this -> success('设置成功',U('Index4/select'));
		}else{
			$this -> error('设置失败');
		}
